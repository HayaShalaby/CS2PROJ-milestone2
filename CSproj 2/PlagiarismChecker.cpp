...
void PlagiarismChecker::compDoc() // We will count the words for refrences and quotes here -rana
{
  ...
  for(int i=0 ; i<database.size() ; i++)
  {
    ...
    cout<<"The percentage of similarity is: "<<(static_cast<double>(totalQuotes+totalPlag+totalRef)/userLines.size())*100<<"%"<<endl;
    cout<<"The percentage of plagiarism is: "<<(static_cast<double>(totalPlag)/userLines.size())*100<<"%"<<endl;
    cout<<"The percentage of Quotes is: "<<(static_cast<double>(totalQuotes)/userLines.size())*100<<"%"<<endl;
    cout<<"The percentage of References is: "<<(static_cast<double>(totalRef)/userLines.size())*100<<"%"<<endl;
...
void PlagiarismChecker::compLines(vector <string> data , vector <string> user) // We will count the words for refrences and quotes here
{
  ...
  for(int j=0; j<user.size(); j++)
  {
          ...
          else if(isQuote(user[j]) && totalQuotes==0)
            {
                totalQuotes++;
            }
          else
